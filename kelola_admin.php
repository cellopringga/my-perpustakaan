...
// Query dengan pencarian aman (Prepared Statement)
if (!empty($keyword)) {
    ...
    $admin_query = "SELECT * FROM admin ORDER BY id ASC";
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="top-banner">
            <div>
                <h1><i class="fa-solid fa-user-shield" style="color: #10b981;"></i> Kelola Admin</h1>
                <p>Total saat ini: <strong><?php echo $total_admin; ?> admin</strong> terdaftar dalam sistem.</p>
            </div>
            <div class="banner-actions">
                <a href="tambah_admin.php" class="btn-add"><i class="fa-solid fa-user-plus"></i> Tambah Admin</a>
        ...
        <div class="card">
            ...
            <div class="search-container">
                <form method="GET" class="search-box">
                    <input type="text" 
                           name="search" 
                           placeholder="Cari username, nama, email, atau NIK..." 
                           value="<?php echo htmlspecialchars($keyword); ?>">
                    <button type="submit"><i class="fa-solid fa-magnifying-glass"></i> Cari</button>
                    <?php if (!empty($keyword)): ?>
                        <a href="kelola_admin.php" class="btn-reset"><i class="fa-solid fa-rotate-left"></i> Reset</a>
                    <?php endif; ?>
                </form>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if (mysqli_num_rows($admin_result) > 0): ?>
                            <?php $no = 1; while ($admin = mysqli_fetch_assoc($admin_result)): 
                                $is_current_admin = ($admin['id'] == $_SESSION['admin_id']);
                            ?>
                                <tr>
                                    <td><?php echo $no++; ?></td>
                                    <td>
                                        ...
                                        <?php if ($is_current_admin): ?>
                                            <span class="badge-current">(Anda)</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo htmlspecialchars($admin['nik'] ?? '-'); ?></td>
                                    <td><?php echo htmlspecialchars($admin['nama_lengkap'] ?? ''); ?></td>
                                    <td><?php echo htmlspecialchars($admin['email'] ?? '-'); ?></td>
                                    <td><?php echo htmlspecialchars($admin['no_telepon'] ?? '-'); ?></td>
                                    <td><?php echo htmlspecialchars($admin['jabatan'] ?? 'Petugas'); ?></td>
                                    <td>
                                        <?php if (isset($admin['is_active']) && $admin['is_active'] == 1): ?>
                                    ...
                                    <td>
                                        <a href="edit_admin.php?id=<?php echo $admin['id']; ?>" class="btn-edit">
                                            <i class="fa-solid fa-pen-to-square"></i> Edit
                                        </a>
                                        <?php if ($is_current_admin): ?>
                                            <span class="btn-disabled" title="Tidak bisa menghapus akun sendiri">
                                                <i class="fa-solid fa-ban"></i> Hapus
                                            </span>
                                        <?php else: ?>
                                            <a href="hapus_admin.php?id=<?php echo $admin['id']; ?>" 
                                               class="btn-delete" 
                                               onclick="return confirm('Yakin ingin menghapus admin &quot;<?php echo htmlspecialchars(addslashes($admin['nama_lengkap'] ?? '')); ?>&quot;?\nData yang dihapus tidak dapat dikembalikan!')">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    <?php if (!empty($keyword)): ?>
                                        Tidak ada admin dengan kata kunci "<strong><?php echo htmlspecialchars($keyword); ?></strong>"
                                    <?php else: ?>
                                        Belum ada data admin. <a href="tambah_admin.php">Tambah admin sekarang</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
